/*
# Create wallet, treasury, and opportunities tables for Flash Arb Engine

1. New Tables
- `arb_wallet`: Stores the encrypted wallet (address, encrypted private key, salt, deployed contracts)
...
CREATE POLICY "anon_select_wallet" ON arb_wallet FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "anon_insert_wallet" ON arb_wallet;
CREATE POLICY "anon_insert_wallet" ON arb_wallet FOR INSERT TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "anon_update_wallet" ON arb_wallet;
CREATE POLICY "anon_update_wallet" ON arb_wallet FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "anon_delete_wallet" ON arb_wallet;
CREATE POLICY "anon_delete_wallet" ON arb_wallet FOR DELETE TO anon, authenticated USING (true);
...
CREATE POLICY "anon_select_treasury" ON arb_treasury FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "anon_insert_treasury" ON arb_treasury;
CREATE POLICY "anon_insert_treasury" ON arb_treasury FOR INSERT TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "anon_update_treasury" ON arb_treasury;
CREATE POLICY "anon_update_treasury" ON arb_treasury FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "anon_delete_treasury" ON arb_treasury;
CREATE POLICY "anon_delete_treasury" ON arb_treasury FOR DELETE TO anon, authenticated USING (true);
