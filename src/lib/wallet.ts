...
export interface WalletState {
  address: string; encryptedKey: string; salt: string;
  isUnlocked: boolean; signer: ethers.Wallet | null;
}
...
export async function generateWallet(password: string): Promise<WalletState> {
  ...
  const { error } = await supabase.from('arb_wallet').insert({ address: wallet.address, encrypted_private_key: encrypted, salt, chain_balances: {}, deployed_contracts: {} });
...
export async function loadWallet(): Promise<{ address: string; encryptedKey: string; salt: string } | null> {
  const { data } = await supabase.from('arb_wallet').select('address, encrypted_private_key, salt').order('created_at', { ascending: false }).limit(1).maybeSingle();
  if (!data) return null;
...
export async function getNativeBalance(address: string, rpcUrl: string): Promise<string> {
  try { const provider = new ethers.JsonRpcProvider(rpcUrl); return ethers.formatEther(await provider.getBalance(address)); }
  catch { return '0.0'; }
}
